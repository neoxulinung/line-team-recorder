...
- **被動記錄**：主題進行中，群組裡的每則文字訊息跟照片訊息都會被記錄下來。其他時間Bot完全不插話，除非有指令觸發。
- **LLM整理文件，每個主題可客製**：定時（也可以用`/整理`手動觸發）讓LLM把新訊息整理進這個主題自己的markdown文件，依照該主題的`organize_prompt`（可在LIFF頁面編輯，有「通用／旅遊規劃」範本下拉選單可以直接套用當作起點）。留預設的通用prompt就是簡單的重點摘要；設一份詳細的（例如比照itineraryManager「時間軸／未定事項／其他資訊」＋來源引用）就能重現類似的整理效果。
- **照片與訊息收回**：主題進行中傳的照片會存進Cloudflare R2，並用markdown語法嵌入文件裡；如果有人收回（unsend）一則訊息，下一次整理時會把文件裡靠那則訊息新增的內容移除或修正。
- **有根據的問答**：`/問 <問題>`只根據這個主題的文件回答，不會用模型自己的知識瞎猜，文件裡沒答案就明講不知道。
- **查核機制**：每次整理完都會跑第二輪LLM檢查，找出有沒有加進沒根據的內容，用`/檢查`查詢。
- **每個主題可選用不同模型**：LIFF頁面可以針對這個主題單獨設定整理／問答／查核各自要用哪個模型（Claude Sonnet/Haiku、GPT-5、GPT-5 mini）——留著「使用預設」就會跟著`src/llm_client.py`目前設定的常數走。
- **LIFF頁面**：一個可分享的網頁，顯示目前文件（可手動編輯、有完整版本歷史），如果該主題開啟了記帳/投票模組也會一併顯示。`/懶人包`會直接回一個連到這個頁面的按鈕；`/開始`成功時也會提醒大家加Bot好友（原因見下方「部署前該知道的設計決策」）並附上一鍵連結。
...
- **要加好友，名字才會正常顯示。** LINE的profile API只有對已經加Bot好友的人才能查到真實顯示名稱（`/開始`會提醒這件事）——還沒加的人會顯示成一串LINE user ID。這件事在對方加好友*之後*整理進文件的內容會自動修正；已經寫進文件的舊內容不會自動變，要嘛靠LIFF頁面的🔄修正名稱顯示按鈕，要嘛等還沒整理過的訊息跑下一次`/整理`。
...
- **Cloudflare R2**存照片，透過它的S3相容API、用`boto3`存取（`src/r2.py`，因為R2沒有官方async client，同步的client包了一層`asyncio.to_thread`）——跟D1的理由一樣，Cloud Run外面用不了Workers binding。
...
- **Cloudflare R2**：免費額度（10GB儲存空間、沒有egress費用）——朋友群組的照片量遠遠碰不到。
...
另外再建立一個R2的API token（dashboard → R2 → Manage R2 API Tokens →「Create API token」，權限選「Object Read & Write」，範圍限定在剛建立的bucket）給`R2_ACCESS_KEY_ID`／`R2_SECRET_ACCESS_KEY`用——這跟上面的D1 token是不同種類的token。在bucket開啟公開存取（dashboard → 你的bucket → Settings → Public access），記下拿到的`pub-*.r2.dev`網址給`R2_PUBLIC_BASE_URL`用。
...
把`.env.example`複製成`.env`，填入所有值（LINE channel secret/token、LLM API key、步驟3拿到的Cloudflare帳號ID／D1 database ID／D1 API token／R2金鑰跟bucket資訊、步驟2的LIFF ID，以及一個隨機產生的`SCHEDULER_SECRET`，例如`openssl rand -hex 32`）。
